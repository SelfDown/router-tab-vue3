...
<script>
import {remove, getCtorId, mapGetters, getTransOpt} from '../util'
...
export default {
  ...
  computed: {
    ...
    hooks() {
      ...
      let h = PAGE_HOOKS.reduce((events, hook) => {
        events['' + hook] = () => this.pageHook(hook)
        return events
      }, {})
  ...
  watch: {
    ...
    $route: {
      handler($route, old) {
        ...
        this.$emit('router-change', type, this.routeMatch)
  ...
  created() {
    console.log("router alive created")
  },
  ...
  methods: {
    ...
    getRouteIndex() {
      let cur = this
      let depth = 0 // 路由深度
      // todo 这里找不到$vnode 被替换成什么了，进过调试我的基本是在第二个
      while (cur && depth < 0) {
    ...
    onTrans() {
      if (this.onRefresh) {
        ...
        this.$emit('router-change', 'create', this.routeMatch)
      }
    },
